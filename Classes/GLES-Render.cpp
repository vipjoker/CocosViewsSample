...
GLESDebugDraw::GLESDebugDraw(float32 ratio, cocos2d::DrawNode *_drawnode)
        : mRatio(ratio), drawNode(_drawnode) {
    this->initShader();
}

void GLESDebugDraw::initShader(void) {
    mShaderProgram = GLProgramCache::getInstance()->getGLProgram(
...
void GLESDebugDraw::DrawPolygon(const b2Vec2 *old_vertices, int vertexCount, const b2Color &color) {
    ...
    drawNode->drawPolygon(vertices, vertexCount, Color4F::RED, .5f, Color4F::BLACK);
...
void
GLESDebugDraw::DrawSolidPolygon(const b2Vec2 *old_vertices, int vertexCount, const b2Color &color) {
    Vec2 *vertices = new Vec2[vertexCount];
    for (int i = 0; i < vertexCount; i++) {
        vertices[i] = Vec2(old_vertices[i].x, old_vertices[i].y);
        vertices[i] *= mRatio;
...
void GLESDebugDraw::DrawSolidCircle(const b2Vec2 &center, float32 radius, const b2Vec2 &axis,
    ...
    Vec2 v(center.x, center.y);
    v *= mRatio;
    Vec2 a(axis.x, axis.y);
    a *= mRatio;

    drawNode->drawCircle(v, radius * mRatio, Vec2(axis.x,axis.y).getAngle() , 32,true, Color4F::BLACK);
...
void GLESDebugDraw::DrawSegment(const b2Vec2 &p1, const b2Vec2 &p2, const b2Color &color) {
    ...
    Vec2 begin(p1.x * mRatio, p1.y * mRatio);
    Vec2 end(p2.x * mRatio, p2.y * mRatio);
    drawNode->drawLine(begin, end, Color4F(color.r,color.g,color.b,1));
